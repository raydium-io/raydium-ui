          ...
          <Steps direction="vertical" progress-dot :current="3">
            <Step>
              ...
              <div slot="description" class="action">
                ...
                <template v-for="farm in farms">
                  <div v-if="farm.farmInfo.version === 2" :key="farm.farmInfo.poolId">
                    <h6 style="display: inline-block">
                      {{ farm.depositBalance.format() }} {{ farm.farmInfo.lp.name }}
                    </h6>
...
import { Button, Icon, Steps } from 'ant-design-vue'
...
export default Vue.extend({
  ...
  methods: {
    ...
    unstakeV3(farmInfo: any, amount: string) {
      ...
      withdraw(conn, wallet, farmInfo, lpAccount, rewardAccount, infoAccount, auxiliaryAccounts, amount)
        .then((txid) => {
          this.$notify.info({
            key,
            message: 'Transaction has been sent',
